...
import nl.lumc.sasc.sentinel.maple.{ MapleSampleRecord, MapleSampleStats }
...
class SamplesAdapterSpec extends Specification
  ...
  class TestSamplesAdapter(mockDb: Fongo) extends SamplesAdapter {
    ...
    private[SamplesAdapterSpec] def find(dbo: MongoDBObject) = mockDb
      .getDB(testDbName)
      .getCollection(MongodbConnector.CollectionNames.pipelineSamples(pipelineName))
      .find(dbo)
  }
  ...
  "storeSamples" should {
    ...
    "fail when the database operation raises any exception" in {
      ...
      mockDb.getCollection(MongodbConnector.CollectionNames.pipelineSamples(pipelineName)) throws new RuntimeException
