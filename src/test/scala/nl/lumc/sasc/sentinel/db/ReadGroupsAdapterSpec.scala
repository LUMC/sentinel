...
import nl.lumc.sasc.sentinel.maple.{ MapleReadGroupRecord, MapleReadGroupStats }
...
class ReadGroupsAdapterSpec extends Specification
  ...
  class TestReadGroupsAdapter(mockDb: Fongo) extends ReadGroupsAdapter {
    ...
    private[ReadGroupsAdapterSpec] def find(dbo: MongoDBObject) = mockDb
      .getDB(testDbName)
      .getCollection(MongodbConnector.CollectionNames.pipelineReadGroups(pipelineName))
      .find(dbo)
  }
  ...
  "storeReadGroups" should {
    ...
    "fail when the database operation raises any exception" in {
      ...
      mockDb.getCollection(MongodbConnector.CollectionNames.pipelineReadGroups(pipelineName)) throws new RuntimeException
