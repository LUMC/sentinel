...
import nl.lumc.sasc.sentinel.utils.{ calcMd5, utcTimeNow, MongodbAccessObject }
 ...
 */
class GentrapRunsProcessor(mongo: MongodbAccessObject)
  ...
  /** Run data container. */
  type RunRecord = GentrapRunRecord
  ...
  private[biopet] def extractReference(runJson: JValue): ReferenceRecord = {
  ...
  private[biopet] def extractAnnotations(runJson: JValue): Seq[AnnotationRecord] =
  ...
  private[biopet] def extractAlnStats(effJson: JValue): GentrapAlignmentStats = {
  ...
  private[biopet] def extractReadStats(readGroupJson: JValue, seqStatKey: String, fastqcKey: String): ReadStats = {
  ...
  private[biopet] def extractReadGroupDocument(readGroupJson: JValue, uploaderId: String, runId: ObjectId,
                                               refId: ObjectId, annotIds: Seq[ObjectId], readGroupName: String,
                                               sampleName: String, runName: Option[String] = None): GentrapReadGroupRecord = {
  ...
  private[biopet] def extractUnits(runJson: JValue, uploaderId: String, runId: ObjectId,
  ...
  private[biopet] def createRun(fileId: ObjectId, refId: ObjectId, annotIds: Seq[ObjectId],
  ...
  def jsonSchemaUrl = "/schemas/biopet/gentrap.json"
  ...
  def processRunUpload(uploaded: FileUpload, uploader: User): Future[RunRecord] =
    ...
    for {
      ...
      ref <- getOrCreateReference(extractReference(runJson))
      annots <- getOrCreateAnnotations(extractAnnotations(runJson))

      annotIds = annots.map(_.annotId)
      runName = (runJson \ "meta" \ "run_name").extractOpt[String]
      (samples, readGroups) <- Future { extractUnits(runJson, uploader.id, fileId, ref.refId, annotIds, runName) }
      _ <- storeSamples(samples)
      _ <- storeReadGroups(readGroups)

      run = createRun(fileId, ref.refId, annotIds, samples, readGroups, uploader, runName)
      _ <- storeRun(run)
    } yield run
}
